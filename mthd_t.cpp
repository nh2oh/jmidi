...
smpte_t time_division_t::get_smpte() const {
	...
	auto p = static_cast<unsigned char*>(static_cast<void*>(&(result.time_code)));
	*p = this->d_[0];
	//result.subframes = static_cast<int8_t>(this->d_[1]);
	p = static_cast<unsigned char*>(static_cast<void*>(&(result.subframes)));
	*p = this->d_[1];
