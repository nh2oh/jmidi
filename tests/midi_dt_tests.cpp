...
//
// bool is_valid_delta_time(int32_t dt_val);
//
// A selection of valid and invalid dt values.  
TEST(delta_time_tests, IsValidDeltaTime) {
	// Valid
	for (const auto& tc : dt_fields_test_set_a) {
		EXPECT_TRUE(is_valid_delta_time(tc.val));
	}
	// Invalid
	for (const auto& tc : dt_test_set_d) {
		EXPECT_FALSE(is_valid_delta_time(tc.dt_input));
	}
	EXPECT_FALSE(is_valid_delta_time(0x0FFFFFFF+1));
	// Largest value of an int32_t; 0x7FFFFFFF+1 overflows
	EXPECT_FALSE(is_valid_delta_time(0x7FFFFFFF));  
}


//
// int32_t to_nearest_valid_delta_time(int32_t dt_val);
//
// A selection of valid and invalid dt values.  
TEST(delta_time_tests, ToNearestValidDeltaTime) {
	// Valid & invalid
	for (const auto& tc : dt_test_set_a) {
		EXPECT_EQ(to_nearest_valid_delta_time(tc.dt_input),tc.ans_value);
	}
	// All are invalid (<0)
	for (const auto& tc : dt_test_set_d) {
		EXPECT_EQ(to_nearest_valid_delta_time(tc.dt_input),tc.ans_value);
...
//
// int32_t delta_time_field_size(int32_t dt_val);
//
// A selection of valid and invalid dt values.  
TEST(delta_time_tests, DeltaTimeFieldSize) {
	// Valid & invalid
	for (const auto& tc : dt_test_set_a) {
		EXPECT_EQ(delta_time_field_size(tc.dt_input),tc.ans_n_bytes);
	}
	// All are invalid (<0)
	for (const auto& tc : dt_test_set_d) {
		EXPECT_EQ(delta_time_field_size(tc.dt_input),tc.ans_n_bytes);
...
TEST(delta_time_tests, WriteDtPositiveValues) {
	...
	for (const auto& tc : dt_test_set_a) {
		// Write
		field.fill(0x00u);
		auto it = write_delta_time(tc.dt_input,field.begin());
		EXPECT_EQ(it-field.begin(),tc.ans_n_bytes);

		// Read
		auto dt = read_delta_time(field.begin(),field.end());
...
TEST(delta_time_tests, ReadWriteDtAllValuesValid) {
	...
	for (const auto& tc : dt_fields_test_set_a) {
		// Read
		auto dt = read_delta_time(tc.field.begin(),tc.field.end());
		...
		// Write
		field.fill(0x00u);
		auto it = write_delta_time(tc.val,field.begin());
...
// to read_delta_time().  Expect dt_field_interpreted.is_valid == false.  
TEST(delta_time_tests, ReadDtTruncateField) {
	for (const auto& tc : dt_fields_test_set_a) {
		...
		auto dt = read_delta_time(tc.field.begin(),
...
TEST(delta_time_tests, AdvanceToDtEnd) {
	for (const auto& tc : dt_fields_test_set_a) {
		auto it = advance_to_dt_end(tc.field.begin(),tc.field.end());
	...
	auto it = advance_to_dt_end(overlong.begin(),overlong.end());
