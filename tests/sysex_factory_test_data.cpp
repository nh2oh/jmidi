...
// ans_pyld_len == payload_in.size(); the factory does not add amn 0xF7u to
// the end of the input payload.  
std::vector<sysex_test_set_a_t> f0f7_tests_no_terminating_f7_on_pyld {
	// "Pathological" input:  empty payload
	{0, {}, 0, 0},
	// Fits in the small-buffer
	{0, {0x01u,0x02u,0x03u,0x04u}, 0, 4},
	// Fits in small buffer; dt.size() => 2
	{0x000000FF, {0x01u,0x02u,0x03u,0x04u}, 0x000000FF, 4},
	// Fits in small buffer; dt.size() => 4
	{0x0FFFFFFF, {0x01u,0x02u,0x03u,0x04u}, 0x0F'FF'FF'FF, 4},
	// Fits in small buffer; dt.size() => 4; note that dt.in > max 
	// allowed dt
	{0x10'00'00'00, {0x01u,0x02u,0x03u,0x04u}, 0x0F'FF'FF'FF, 4},
...
// ans_pyld_len == payload_in.size(); the factory does not add an 0xF7u to 
// the end of the input payload.  
