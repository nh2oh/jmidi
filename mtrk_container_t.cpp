...
raw_mtrk_iterator_t::raw_mtrk_iterator_t(const unsigned char *p, unsigned char s) {
	this->p_ = p;
	this->s_ = s;
}
mtrk_event_t raw_mtrk_iterator_t::operator*() const {
...
raw_mtrk_iterator_t& raw_mtrk_iterator_t::operator++() {
...
bool raw_mtrk_iterator_t::operator==(const raw_mtrk_iterator_t& rhs) const {
...
mtrk_event_view_t raw_mtrk_iterator_t::operator->() const {
...
raw_mtrk_iterator_t mtrk_view_t::begin() const {
	...
	return raw_mtrk_iterator_t::raw_mtrk_iterator_t(this->p_+8,0x00u);
}
raw_mtrk_iterator_t mtrk_view_t::end() const {
	// Note that i am supplying an invalid midi status byte for this 
	// one-past-the-end iterator.  
	return raw_mtrk_iterator_t::raw_mtrk_iterator_t(this->p_+this->size(),0x00u);
}
...
std::string print(const mtrk_view_t& mtrk) {
	std::string s {};
	for (raw_mtrk_iterator_t it=mtrk.begin(); it!=mtrk.end(); ++it) {
		auto ev = *it;
